...
select f.nome as fornecedor,
	count(p.id) as qtde_total_produtos
	from produto as p
	left join fornecedor as f
	on p.id_fornecedor = f.id
	group by f.nome;
...
select f.nome as fornecedor,
	sum(c.vlr_compra) as valor_total_compras
	from compra c
...
select p.nome as produtp,
	sum(c.qtde) as quatidade_total
	from compra c
	inner join produto as p on c.id_produto = p.id
	group by p.nome;
...
select f.nome as fornecedor, p.nome as produto,
	sum(c.vlr_compra) as valor_total_comprado
	from compra c
	inner join produto as p on c.id_produto = p.id
	inner join fornecedor as f on p.id_fornecedor = f.id
...
select c.nf, f.nome as fornecedor, p.nome as produto
	from compra c
	inner join produto as p on c.id_produto = p.id
	inner join fornecedor as f on p.id_fornecedor = f.id
